...
function checkOrderList () {
  return Axios.get('/qui/order/query')
}
// 获取全部订单数量   y 修改
function huoququanbu () {
  return Axios.get('/qui/order/queryQ')
}
// 按条件获取订单数量   y 修改
function antiaojian (state) {
  return Axios.get('/qui/order/orderQ?orderDetailStatus=' + state)
}
// 创建订单  这个是创建订单
...
function queryorder (state) {
  return Axios.get('/qui/order/queryOrder?orderDetailStatus=' + state)
}
// 快速打单
function print (id) {
  let params = {
...
export default {
  ...
  print: print,
  update: update,
  huoququanbu: huoququanbu,
  antiaojian: antiaojian
}
